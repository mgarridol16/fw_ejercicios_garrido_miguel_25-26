...
export class ViewService {
    renderizarListado(contenedor, recetas, estaLogueado, esZonaPrivada = false) {
    ...
    mostrarAviso(contenedor, mensaje, tipo) {
        contenedor.innerHTML = "";
        const html = `<div class="alert alert-${tipo}" role="alert">
    ${mensaje}</div>`;
        contenedor.innerHTML = html;
    }
    renderizarDetalles(contenedor, receta) {
        contenedor.innerHTML = "";
    ...
    renderizarPlanesSemanales(contenedor, planes, idSemanaActual) {
        contenedor.innerHTML = "";
        if (planes.length === 0) {
